/**
 * PDF Template Generator
 * Generates clean, professional HTML for PDF reports
 */

const generateHtml = (analysis, userInfo) => {
  // Safe destructuring with defaults
  const {
    primaryCareerField = { name: "Karyera Sahəsi", description: "Analiz edilir", matchPercentage: 0 },
    topCareerFields = [],
    strengths = [],
    areasToImprove = [],
    recommendedSkills = [],
    suggestedJobRoles = [],
    sixMonthRoadmap = {},
    motivationalMessage = "Hər böyük səyahət bir addımla başlayır!",
    selectedFieldComparison = null,
  } = analysis || {};

  // Ensure arrays are actually arrays
  const safeTopCareerFields = Array.isArray(topCareerFields) ? topCareerFields : [];
  const safeStrengths = Array.isArray(strengths) ? strengths : [];
  const safeAreasToImprove = Array.isArray(areasToImprove) ? areasToImprove : [];
  const safeRecommendedSkills = Array.isArray(recommendedSkills) ? recommendedSkills : [];
  const safeSuggestedJobRoles = Array.isArray(suggestedJobRoles) ? suggestedJobRoles : [];
  const safeSixMonthRoadmap = sixMonthRoadmap && typeof sixMonthRoadmap === 'object' ? sixMonthRoadmap : {};

  const currentDate = new Date().toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });

  return `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Career Assessment Report - ${userInfo.name}</title>
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
    
    * {
      margin: 0;
      padding: 0;
      box-sizing: border-box;
    }
    
    body {
      font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
      font-size: 11pt;
      line-height: 1.6;
      color: #1a1a2e;
      background: #ffffff;
    }
    
    .container {
      max-width: 100%;
      padding: 0;
    }
    
    /* Header */
    .header {
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      color: white;
      padding: 40px 30px;
      margin: -20mm -15mm 30px -15mm;
      text-align: center;
    }
    
    .header h1 {
      font-size: 28pt;
      font-weight: 700;
      margin-bottom: 10px;
      letter-spacing: -0.5px;
    }
    
    .header .subtitle {
      font-size: 12pt;
      opacity: 0.9;
      font-weight: 300;
    }
    
    .header .user-info {
      margin-top: 20px;
      padding-top: 20px;
      border-top: 1px solid rgba(255,255,255,0.3);
    }
    
    .header .user-name {
      font-size: 18pt;
      font-weight: 600;
    }
    
    .header .report-date {
      font-size: 10pt;
      opacity: 0.8;
      margin-top: 5px;
    }
    
    /* Section Styling */
    .section {
      margin-bottom: 30px;
      page-break-inside: avoid;
    }
    
    .section-title {
      font-size: 14pt;
      font-weight: 700;
      color: #667eea;
      margin-bottom: 15px;
      padding-bottom: 8px;
      border-bottom: 2px solid #667eea;
      display: flex;
      align-items: center;
      gap: 10px;
    }
    
    .section-icon {
      width: 24px;
      height: 24px;
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      border-radius: 6px;
      display: inline-flex;
      align-items: center;
      justify-content: center;
      color: white;
      font-size: 12pt;
    }
    
    /* Primary Career Field */
    .primary-field {
      background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%);
      border-radius: 12px;
      padding: 25px;
      text-align: center;
      margin-bottom: 20px;
    }
    
    .primary-field h3 {
      font-size: 20pt;
      color: #1a1a2e;
      margin-bottom: 10px;
    }
    
    .match-percentage {
      font-size: 36pt;
      font-weight: 700;
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
    }
    
    .match-label {
      font-size: 10pt;
      color: #666;
      text-transform: uppercase;
      letter-spacing: 1px;
    }
    
    .primary-description {
      margin-top: 15px;
      color: #555;
      font-size: 11pt;
    }
    
    /* Top Career Fields Table */
    .career-table {
      width: 100%;
      border-collapse: collapse;
      margin-top: 10px;
    }
    
    .career-table th {
      background: #667eea;
      color: white;
      padding: 12px 15px;
      text-align: left;
      font-weight: 600;
      font-size: 10pt;
    }
    
    .career-table td {
      padding: 12px 15px;
      border-bottom: 1px solid #eee;
    }
    
    .career-table tr:nth-child(even) {
      background: #f8f9fa;
    }
    
    .career-table .rank {
      width: 50px;
      text-align: center;
      font-weight: 700;
      color: #667eea;
    }
    
    .career-table .percentage {
      width: 80px;
      text-align: center;
    }
    
    .percentage-badge {
      display: inline-block;
      padding: 4px 12px;
      border-radius: 20px;
      font-weight: 600;
      font-size: 10pt;
    }
    
    .percentage-high {
      background: #d4edda;
      color: #155724;
    }
    
    .percentage-medium {
      background: #fff3cd;
      color: #856404;
    }
    
    .percentage-low {
      background: #f8d7da;
      color: #721c24;
    }
    
    /* Lists */
    .styled-list {
      list-style: none;
      padding: 0;
    }
    
    .styled-list li {
      padding: 10px 15px;
      margin-bottom: 8px;
      background: #f8f9fa;
      border-radius: 8px;
      border-left: 4px solid #667eea;
      font-size: 10pt;
    }
    
    .styled-list li strong {
      color: #667eea;
    }
    
    /* Skills Cards */
    .skills-grid {
      display: grid;
      grid-template-columns: repeat(2, 1fr);
      gap: 15px;
    }
    
    .skill-card {
      background: #f8f9fa;
      border-radius: 10px;
      padding: 15px;
      border: 1px solid #eee;
    }
    
    .skill-card h4 {
      color: #1a1a2e;
      font-size: 11pt;
      margin-bottom: 8px;
      display: flex;
      align-items: center;
      gap: 8px;
    }
    
    .skill-importance {
      font-size: 8pt;
      padding: 2px 8px;
      border-radius: 10px;
      font-weight: 600;
    }
    
    .importance-high {
      background: #667eea;
      color: white;
    }
    
    .importance-medium {
      background: #ffc107;
      color: #1a1a2e;
    }
    
    .importance-low {
      background: #6c757d;
      color: white;
    }
    
    .skill-card p {
      font-size: 9pt;
      color: #666;
      line-height: 1.5;
    }
    
    /* Job Roles */
    .job-role {
      background: white;
      border: 1px solid #e0e0e0;
      border-radius: 10px;
      padding: 15px;
      margin-bottom: 12px;
    }
    
    .job-role h4 {
      color: #1a1a2e;
      font-size: 12pt;
      margin-bottom: 5px;
    }
    
    .job-role p {
      font-size: 10pt;
      color: #666;
      margin-bottom: 8px;
    }
    
    .job-meta {
      display: flex;
      gap: 15px;
      font-size: 9pt;
    }
    
    .job-meta span {
      display: inline-flex;
      align-items: center;
      gap: 5px;
      color: #888;
    }
    
    .demand-high {
      color: #28a745 !important;
    }
    
    .demand-medium {
      color: #ffc107 !important;
    }
    
    .demand-low {
      color: #dc3545 !important;
    }
    
    /* Roadmap */
    .roadmap {
      position: relative;
    }
    
    .roadmap-month {
      display: flex;
      margin-bottom: 20px;
      position: relative;
    }
    
    .month-marker {
      width: 60px;
      text-align: center;
      flex-shrink: 0;
    }
    
    .month-number {
      width: 40px;
      height: 40px;
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      border-radius: 50%;
      display: flex;
      align-items: center;
      justify-content: center;
      color: white;
      font-weight: 700;
      font-size: 14pt;
      margin: 0 auto;
    }
    
    .month-content {
      flex: 1;
      background: #f8f9fa;
      border-radius: 10px;
      padding: 15px 20px;
      margin-left: 15px;
    }
    
    .month-content h4 {
      color: #667eea;
      font-size: 11pt;
      margin-bottom: 10px;
    }
    
    .month-content ul {
      list-style: none;
      padding: 0;
    }
    
    .month-content li {
      padding: 5px 0;
      font-size: 10pt;
      color: #555;
      position: relative;
      padding-left: 20px;
    }
    
    .month-content li:before {
      content: "✓";
      position: absolute;
      left: 0;
      color: #667eea;
      font-weight: bold;
    }
    
    /* Comparison Box */
    .comparison-box {
      background: linear-gradient(135deg, #fff9e6 0%, #fff3cd 100%);
      border: 2px solid #ffc107;
      border-radius: 12px;
      padding: 20px;
      margin-top: 20px;
    }
    
    .comparison-box h4 {
      color: #856404;
      font-size: 12pt;
      margin-bottom: 10px;
    }
    
    .comparison-box p {
      font-size: 10pt;
      color: #666;
      line-height: 1.6;
    }
    
    .fit-badge {
      display: inline-block;
      padding: 5px 15px;
      border-radius: 20px;
      font-weight: 600;
      font-size: 10pt;
      margin: 10px 0;
    }
    
    .fit-good {
      background: #d4edda;
      color: #155724;
    }
    
    .fit-not-good {
      background: #f8d7da;
      color: #721c24;
    }
    
    /* Motivational Message */
    .motivation-box {
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      color: white;
      border-radius: 12px;
      padding: 30px;
      text-align: center;
      margin-top: 30px;
    }
    
    .motivation-box h3 {
      font-size: 14pt;
      margin-bottom: 15px;
      font-weight: 600;
    }
    
    .motivation-box p {
      font-size: 12pt;
      line-height: 1.8;
      font-style: italic;
      opacity: 0.95;
    }
    
    /* Footer */
    .footer {
      margin-top: 40px;
      padding-top: 20px;
      border-top: 1px solid #eee;
      text-align: center;
      font-size: 9pt;
      color: #888;
    }
    
    .footer .brand {
      font-weight: 600;
      color: #667eea;
    }
    
    /* Page Break Helpers */
    .page-break {
      page-break-before: always;
    }
    
    @media print {
      .section {
        page-break-inside: avoid;
      }
    }
  </style>
</head>
<body>
  <div class="container">
    <!-- Header -->
    <div class="header">
      <h1>Career Assessment Report</h1>
      <p class="subtitle">Comprehensive Career Analysis & Guidance</p>
      <div class="user-info">
        <p class="user-name">${userInfo.name}</p>
        <p class="report-date">Generated on ${currentDate}</p>
      </div>
    </div>
    
    <!-- Primary Career Field -->
    <div class="section">
      <div class="primary-field">
        <p class="match-label">Your Ideal Career Field</p>
        <h3>${primaryCareerField.name}</h3>
        <p class="match-percentage">${primaryCareerField.matchPercentage}%</p>
        <p class="match-label">Match Score</p>
        <p class="primary-description">${primaryCareerField.description}</p>
      </div>
    </div>
    
    <!-- Top 3 Career Fields -->
    <div class="section">
      <h2 class="section-title">
        <span class="section-icon">📊</span>
        Top Career Fields
      </h2>
      <table class="career-table">
        <thead>
          <tr>
            <th class="rank">#</th>
            <th>Career Field</th>
            <th class="percentage">Match</th>
            <th>Why It Suits You</th>
          </tr>
        </thead>
        <tbody>
          ${safeTopCareerFields.map(field => `
            <tr>
              <td class="rank">${field.rank || 1}</td>
              <td><strong>${field.name || 'Karyera Sahəsi'}</strong></td>
              <td class="percentage">
                <span class="percentage-badge ${(field.matchPercentage || 0) >= 80 ? 'percentage-high' : (field.matchPercentage || 0) >= 60 ? 'percentage-medium' : 'percentage-low'}">
                  ${field.matchPercentage || 0}%
                </span>
              </td>
              <td>${field.reason || 'Sizə uyğundur'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    </div>
    
    <!-- Strengths -->
    <div class="section">
      <h2 class="section-title">
        <span class="section-icon">💪</span>
        Your Strengths
      </h2>
      <ul class="styled-list">
        ${safeStrengths.map(strength => `<li>✨ ${strength}</li>`).join('')}
      </ul>
    </div>
    
    <!-- Areas to Improve -->
    <div class="section">
      <h2 class="section-title">
        <span class="section-icon">🎯</span>
        Areas to Improve
      </h2>
      <ul class="styled-list">
        ${safeAreasToImprove.map(area => `<li>📈 ${area}</li>`).join('')}
      </ul>
    </div>
    
    <!-- Recommended Skills -->
    <div class="section">
      <h2 class="section-title">
        <span class="section-icon">🛠️</span>
        Recommended Skills to Develop
      </h2>
      <div class="skills-grid">
        ${safeRecommendedSkills.map(skill => `
          <div class="skill-card">
            <h4>
              ${skill.skill || skill.name || 'Bacarıq'}
              <span class="skill-importance importance-${(skill.importance || 'medium').toLowerCase()}">${skill.importance || 'Medium'}</span>
            </h4>
            <p>${skill.description || 'Bu bacarıq sizə kömək edəcək'}</p>
          </div>
        `).join('')}
      </div>
    </div>
    
    <div class="page-break"></div>
    
    <!-- Suggested Job Roles -->
    <div class="section">
      <h2 class="section-title">
        <span class="section-icon">💼</span>
        Suggested Job Roles
      </h2>
      ${safeSuggestedJobRoles.map(job => `
        <div class="job-role">
          <h4>${job.title || job.name || 'Vəzifə'}</h4>
          <p>${job.description || 'Bu vəzifə sizə uyğundur'}</p>
          <div class="job-meta">
            <span>💰 ${job.salaryRange || '1000-3000 AZN'}</span>
            <span class="demand-${(job.demandLevel || 'high').toLowerCase()}">📈 ${job.demandLevel || 'High'} Demand</span>
          </div>
        </div>
      `).join('')}
    </div>
    
    <!-- 6-Month Roadmap -->
    <div class="section">
      <h2 class="section-title">
        <span class="section-icon">🗺️</span>
        Your 6-Month Career Roadmap
      </h2>
      <div class="roadmap">
        ${Object.entries(safeSixMonthRoadmap).map(([key, month], index) => `
          <div class="roadmap-month">
            <div class="month-marker">
              <div class="month-number">${index + 1}</div>
            </div>
            <div class="month-content">
              <h4>${month && month.title ? month.title : `Ay ${index + 1}`}</h4>
              <ul>
                ${(month && Array.isArray(month.tasks) ? month.tasks : []).map(task => `<li>${task}</li>`).join('')}
              </ul>
            </div>
          </div>
        `).join('')}
      </div>
    </div>
    
    ${selectedFieldComparison ? `
    <!-- Field Comparison -->
    <div class="section">
      <div class="comparison-box">
        <h4>📋 Your Selected Field Analysis: ${selectedFieldComparison.selectedField}</h4>
        <span class="fit-badge ${selectedFieldComparison.isGoodFit ? 'fit-good' : 'fit-not-good'}">
          ${selectedFieldComparison.isGoodFit ? '✓ Good Fit' : '⚠ Consider Alternatives'} - ${selectedFieldComparison.fitPercentage}% Match
        </span>
        <p><strong>Comparison:</strong> ${selectedFieldComparison.comparison}</p>
        <p style="margin-top: 10px;"><strong>Our Recommendation:</strong> ${selectedFieldComparison.recommendation}</p>
      </div>
    </div>
    ` : ''}
    
    <!-- Motivational Message -->
    <div class="motivation-box">
      <h3>💫 A Message For You</h3>
      <p>"${motivationalMessage}"</p>
    </div>
    
    <!-- Footer -->
    <div class="footer">
      <p>This report was generated by <span class="brand">WAYME Career Assessment Platform</span></p>
      <p>© ${new Date().getFullYear()} WAYME. All rights reserved.</p>
      <p style="margin-top: 10px; font-size: 8pt;">
        This assessment is based on AI analysis and should be used as guidance only. 
        We recommend consulting with career professionals for personalized advice.
      </p>
    </div>
  </div>
</body>
</html>
  `.trim();
};

module.exports = {
  generateHtml,
};
